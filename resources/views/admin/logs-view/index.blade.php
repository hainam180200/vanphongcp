                        ...
                        <div class="list-group div-scroll">
                            @foreach($data['folders'] as $folder)
                                <div class="list-group-item">
                                    <?php
                                    \Rap2hpoutre\LaravelLogViewer\LaravelLogViewer::DirectoryTreeStructure( $data['storage_path'],$data['structure'] );
                                    ?>
                            ...
                            <select class="form-control selectlogview" name="selectlogview">
                                @foreach($data['files'] as $file)
                                    <option value="?l={{ \Illuminate\Support\Facades\Crypt::encrypt($file) }}"><a href="?l={{ \Illuminate\Support\Facades\Crypt::encrypt($file) }}"
            ...
            <div class="col-12 table-container logviewrow__col2">
                ...
                <div class="p-3">
                    @if($data['current_file'])
                        ...
                        -
                        <a id="clean-log" href="?clean={{ \Illuminate\Support\Facades\Crypt::encrypt($data['current_file']) }}{{ ($data['current_folder']) ? '&f=' . \Illuminate\Support\Facades\Crypt::encrypt($data['current_folder']) : '' }}">
                        ...
                        @if(count($data['files']) > 1)
                            -
                            <a id="delete-all-log" href="?delall=true{{ ($data['current_folder']) ? '&f=' . \Illuminate\Support\Facades\Crypt::encrypt($data['current_folder']) : '' }}">
