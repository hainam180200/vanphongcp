...
@section('content')
                ...
                <div class="row">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control datatable-input" id="order_id" value="{{request('order_id')}}" placeholder="{{__('Order')}}">
                        </div>
                    </div>
                    {{--started_at--}}
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        ...
                        <div class="form-group col-12 col-sm-6 col-lg-3">
                            <div class="input-group">
                                ...
                                {{Form::select('gate_id',[''=>'-- Tất cả nhà cung cấp --']+config('hook.gate_id'),old('gate_id',request('gate_id')),array('id'=>'gate_id','class'=>'form-control datatable-input',))}}
                            </div>
                        </div>
 ...
 <!-- delete item Modal -->
 <div class="modal fade" id="ReCallback">
	<div class="modal-dialog">
		<div class="modal-content">
			{{Form::open(array('route'=>array('admin.hook.callback'),'class'=>'form-horizontal','id'=>'form-callback','method'=>'POST'))}}
			...
			<div class="modal-body">
				{{__('Bạn thực sự muốn callback lại đơn hàng này?')}}
			</div>
			<div class="modal-footer">
				<input type="hidden" name="id" class="id" value=""/>
				<button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('Hủy')}}</button>
				<button type="submit" class="btn btn-danger m-btn m-btn--custom btn-submit-custom" data-form="form-callback">{{__('Đồng ý')}}</button>
			</div>
			{{ Form::close() }}
		</div>
	</div>
...
@section('scripts')
    ...
    <script>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {data: 'id', title: 'ID'},
                        {
                            data: 'created_at', title: '{{__('Thời gian lấy dữ liệu')}}',render: function (data, type, row) {
                                return row.created_at;
                            }
                        },
                        {
                            data: 'gate_id', title: '{{__('Nhà cung cấp')}}',render: function (data, type, row) {
                                return row.gate_id+ '<br /> '+row.tranid;
                            }
                        },
                        {
                            data: 'title', title: '{{__('Đầu vào')}}',render: function (data, type, row) {
                                return row.title;
                            }
                        },
                        {data: 'price', title: 'Số tiền'},
                        {data: 'user', title: 'Shop'},
                        {data: 'order', title: 'Mã đơn gửi'},
                        {data: 'description', title: 'Nội dung gửi'},
                        {data: 'bank', title: 'Ngân hàng'},
                        {
                            data: 'payment_type', title: '{{__('Loại')}}',render: function (data, type, row) {
                                if (row.payment_type == 1) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2  label-success \">" + "{{config('hook.payment_type.1')}}" + "</span>";
                                } else if(row.payment_type == 0) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-danger \">" + "{{config('hook.payment_type.0')}}" + "</span>";
                                }
                            }
                        },
                        // {
                        //     data: 'content', title: '{{__('Msg Callback')}}',render: function (data, type, row) {
                        //         return "<p class='msg-"+row.id+"'>"+row.content+"</p>";
                        //     }
                        // },
        ...
        jQuery(document).ready(function () {
            ...
            $('#ReCallback').on('show.bs.modal', function(e) {
                var id = $(e.relatedTarget).attr('rel')
                $('#ReCallback .id').attr('value', id);
            });
            ...
            $('#form-callback').submit(function (e) {
                ...
                $.ajax({
                    ...
                    error: function (data) {
                        toast("Có lỗi phát sinh vui lòng liên hệ QTV để xử lý", 'error');
                    },
                    complete: function (data) {
                        $('#ReCallback').modal('hide')
                        btnSubmit.removeClass('spinner spinner-right spinner-white pr-15');
