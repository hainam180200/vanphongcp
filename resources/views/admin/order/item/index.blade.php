                ...
                <div class="row">
                    ...
                    <div class="form-group col-12 col-sm-6 col-lg-3">
                        <div class="input-group">
                            ...
                            {{Form::select('status',[''=>'-- Tất cả trạng thái --']+config('order.status'),old('status', isset($data) ? $data->status : null),array('id'=>'status','class'=>'form-control datatable-input',))}}
                        </div>
                    </div>
            ...
            var initTable1 = function () {
                ...
                datatable = $('#kt_datatable').DataTable({
                    ...
                    columns: [
                        ...
                        {
                            data: 'product', title: '{{__('Sản phẩm')}}',
                            render: function (data, type, row) {
                                let html = '';
                                var result = JSON.parse(row.product);
                                $.each(result,function(key,value){
                                    html += '- '+value.item.title+' ||-- [ Số lượng: '+ value.quantity+ ' ]';
                                    html += '<br />';
                                })
                        ...
                        {data: 'real_received_price', title: '{{__('Số tiền thanh toán')}}'},
                        {data: 'type', title: '{{__('PT Thanh toán')}}'},
                        {
                            data: 'status', title: '{{__('Trạng thái')}}',
                            render: function (data, type, row) {

                                if (row.status == 0) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2  label-danger \">" + "{{config('order.status.0')}}" + "</span>";
                                } 
                                else if (row.status == 1) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-dark \">" + "{{config('order.status.1')}}" + "</span>";
                                } 
                                else if (row.status == 2) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-warning \">" + "{{config('order.status.2')}}" + "</span>";
                                } 
                                else if (row.status == 3) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-info \">" + "{{config('order.status.3')}}" + "</span>";
                                } 
                                else if (row.status == success) {
                                    return "<span class=\"label label-pill label-inline label-center mr-2 label-success \">" + "{{config('order.status.3')}}" + "</span>";
                                } 
                                else {
                                    return "";
                                }
