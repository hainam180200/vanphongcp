...
//            }
    ...
    public function create(Request $request)
    {
        ...
        return view('admin.permission.create_edit', compact('dataCategory'));
    }
		...
		foreach ($menu as $item)
			if ($item->parent_id == $parent_id) {
              ...
              <div class='nested-list-content'>";
				if($parent_id!=0){
                    $result.="<div class=\"m-checkbox\">
                                    ...
                                    <input  type=\"checkbox\" rel=\"{$item->id}\" class=\"children_of_{$item->parent_id}\"  >
                                      <span></span> ".HTML::entities($item->title).' - ['.HTML::entities($item->name)."]
                                    </label>
                                </div>";
				...
				$result .= "<a href='#' class='btn btn-sm btn-primary edit_toggle' data-url='" . route("admin.permission.edit",$item->id) . "' rel='{$item->id}' >Sửa</a>
